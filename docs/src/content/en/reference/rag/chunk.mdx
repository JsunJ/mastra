...
**Important:** The HTML strategy has different behavior depending on which option you use:
- When using `headers`: Content is split based on HTML structure only, ignoring `maxSize` and `overlap` parameters
- When using `sections`: Content respects `maxSize` and `overlap` parameters for further chunking
...
**Important:** When using the `headers` option, the markdown strategy ignores `maxSize` and `overlap` parameters. Content is split based on the markdown header structure. To use size-based chunking with markdown, omit the `headers` parameter.
...
The Latex strategy uses only the base chunking parameters listed above. It provides LaTeX-aware splitting optimized for mathematical and academic documents.
