...
export interface GeminiLiveVoiceConfig {
  ...
  debug?: boolean;
  /** Path to service account key file for Vertex AI authentication */
  serviceAccountKeyFile?: string;
  /** Service account email for impersonation (Vertex AI) */
  serviceAccountEmail?: string;
  /** Custom OAuth scopes for Vertex AI (defaults to cloud-platform) */
  authScopes?: string[];
  /** Access token for direct authentication (bypasses OAuth flow) */
  accessToken?: string;
  /** Response modality - 'TEXT' for text responses, 'AUDIO' for audio responses (default: 'TEXT') */
...
export interface GeminiLiveVoiceOptions {
  ...
  /** Timeout for transcription requests in milliseconds (default: 30000) */
  timeout?: number;
}
...
export interface GeminiLiveEventMap {
  ...
  toolCallStart: { toolCallId: string; toolName: string; args: any };
  /** Tool call execution completed */
  toolCallResult: { toolCallId: string; toolName: string; args: any; result: any };
  /** Tool call execution failed */
  toolCallError: { toolCallId: string; toolName: string; args: any; error: string };
